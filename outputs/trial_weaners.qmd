...
with(subset(features_dt, animal_id == "FFFB4759B2C2" & doy %in% c(278:290)),
     plot(temperature ~ timestamp, type = "l", col = "grey", lwd = 1.6, xlab = "", ylab = "\nTag temperature (ºC)"))
with(subset(features_dt, animal_id == "FFFB4759B2C2" & doy %in% c(278:290)),
     lines(Rolling_Avg_3h ~ timestamp, col = "orange", lwd = 4))
# with(subset(features_dt, animal_id == "D0C28D4DE6A1" & doy %in% c(278:290)),
...
title("Ear tag temperature (dmac B2C2)")

# Add the legend outside the plot
legend("topright", inset = c(-0.2, 0), legend = c("Obs", "3h","12h", "30h", "48h"),
       col = c("grey", "orange", "darkblue", "purple2", "firebrick"), 
       lwd = rep(4, 7), xpd = TRUE, title = "Rolling mean")

# "forestgreen""steelblue","6h","24h",
...
clean_dt[, target := condition]
# clean_dt[, target := ifelse(condition == "Healthy", 0, 1)]

ggplot(clean_dt, aes(x = Rolling_min_48h)) +
  geom_density(aes(fill = factor(target)), alpha = .7) +
  labs(title = "Rolling min 48h temperature distribution", 
       subtitle = "Batch 2 - weaner room",
       y = "Density\n", 
       x = "\nEar tag temperature (°C)") +
  scale_fill_viridis_d(option = "D", name = "Condition") +
  theme_light(base_size = 16)

ggplot(clean_dt, aes(x = Rolling_max_48h)) +
  geom_density(aes(fill = factor(target)), alpha = .7) +
  labs(title = "Rolling max 48h temperature distribution", 
       subtitle = "Batch 2 - weaner room",
       y = "Density\n", 
       x = "\nEar tag temperature (°C)") +
  scale_fill_viridis_d(option = "D", name = "Condition") +
...
ggplot(clean_dt, aes(x = Rolling_range_48h)) +
  geom_density(aes(fill = factor(target)), alpha = .7) +
  labs(title = "Rolling range 48h temperature distribution", 
       subtitle = "Batch 2 - weaner room",
       y = "Density\n", 
       x = "\nEar tag temperature (°C)") +
  scale_fill_viridis_d(option = "D", name = "Condition") +
...
arrow::write_parquet(clean_dt, here::here("data/trial_3Ps_batch2_cleaned_data.parquet"))
